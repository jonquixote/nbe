...
| **Step 2: "as JSON layouts in `templates/graphics/`"** | No such directory exists in the tree. Either create it or place templates in the show package, which is where `templateId` references resolve from today (`main.rs:566`). Decide in the upgrade pass; do not leave both readings open. |
