...
| **Stream reconnect is automatic** — transport loss surfaces `Reconnecting`, the publisher redials with no operator action, and `droppedFramesTotal` is unchanged across kill + redial | `prompt10_rtmp::reconnect_kill_midstream_then_live_again_without_operator_action`: in-process RTMP double over TCP loopback, killed mid-stream and restarted on the same port (live → reconnecting → live, no directive) |
| **Stream drops are counted, never silent** — a full channel sheds stream frames only (never record skips, never View drops); `streamBufferMs` is buffered bytes through the §9.4 envelope, nonzero under stall, zero drained | `zerocopy_g1` guard (stream drops counted two independent ways; record skips and View drops stay 0) + `prompt10_rtmp` survival/buffer/tick tests (non-blocking publishes, bytes→ms formula, tick wiring) |
